...
struct FileItemPickerView: View {
    ...
    var body: some View {
        NavigationStack {
            List {
                if filtered.isEmpty {
                    ContentUnavailableView("No files found", systemImage: "doc", description: Text("Import files first, then attach them to invoices."))
                } else {
