...
enum ContractPDFGenerator {
    ...
    static func makePDFData(contract: Contract, business: BusinessProfile?) -> Data {
        ...
        return renderer.pdfData { ctx in
            ...
            func drawSignatureBlocks() {
                ...
                // Once signed, the client's name and date go on their lines.
                if let signed = ContractPDFGenerator.clientSignature(for: contract) {
                    drawText(signed.name, font: .systemFont(ofSize: 15, weight: .semibold),
                             rect: CGRect(x: leftX, y: y + 2, width: colWidth, height: 20),
                             color: primaryText)
                    drawText(signed.method, font: textFont,
                             rect: CGRect(x: leftX, y: y + 28, width: colWidth, height: 14),
                             color: secondaryText)
                    drawText(signed.date, font: textFont,
                             rect: CGRect(x: leftX, y: y + 48, width: colWidth, height: 14),
                             color: primaryText)
                }
            ...
            // No status here: this copy goes out with the contract and stays
            // "Sent" forever, even on the signed copy the client keeps.
            let meta = "Prepared \(formatDate(contract.sentAt ?? contract.updatedAt))"
            drawText(meta,
                     font: .systemFont(ofSize: 10.5, weight: .regular),
            ...
            // Footer, at the bottom of the last page. The signature block
            // already made room, so it never needs a page of its own.
            let footerY = pageHeight - 30
            drawText("Powered by SmallBiz Workspace",
    ...
    /// The client's side of the signature block, or nil until signed.
    static func clientSignature(for contract: Contract) -> (name: String, method: String, date: String)? {
        guard contract.status == .signed else { return nil }
        let name = contract.signedByName.trimmingCharacters(in: .whitespacesAndNewlines)
        guard !name.isEmpty else { return nil }
        let method = contract.signedMethod == "in_person" ? "Signed in person" : "Signed electronically"
        let date = contract.signedAt.map { $0.formatted(date: .abbreviated, time: .omitted) } ?? ""
        return (name, method, date)
    }
