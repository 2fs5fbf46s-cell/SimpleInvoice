...
struct OutstandingBalancesView: View {
    ...
    private var rows: [ClientBalanceRowModel] {
        ClientBalanceAggregation.rows(invoices: invoices, scope: .outstanding)
    }
    ...
    var body: some View {
        ...
        return ZStack {
            ...
            if balanceRows.isEmpty {
                ContentUnavailableView(
                    ...
                    description: Text("All sent invoices are paid for this business.")
                )
            } else {
                ScrollView {
                    SBWCardContainer {
                            ...
                            ForEach(Array(balanceRows.enumerated()), id: \.element.id) { index, row in
                                NavigationLink {
                                    ...
                                    balanceRow(
                                        ...
                                        countText: "\(row.invoiceCount) unpaid invoice\(row.invoiceCount == 1 ? "" : "s")"
                                    )
                                }
                                .buttonStyle(.plain)
        ...
        .navigationTitle("Outstanding Balances")
